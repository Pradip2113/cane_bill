...
class VendorPaymentRelease(Document):
	...
	def get_list(self):
		...
		for d in self.get("vpe_table"):
			vendor_amt_info_child = frappe.get_all("Child Vendor Payment Entry",filters={"parent":d.vendor_payment_entry ,"pay_r_s":0,"docstatus":1,'check':1},fields=["vendor_id","vendor_name","address","total_amount","type","contract_id","account_details","bank_name","name"])
	...
	def sort_bank(self):
		...
		for i in self.get("vendor_amount_information"):
			if i.check:
				...
				transfer_type = "self_bank_transfer" if self.bank == i.acc_details else "different_bank_transfer_vpe"
